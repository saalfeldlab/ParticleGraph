...
from ParticleGraph.utils import to_numpy, set_device, symmetric_cutoff, norm_velocity, norm_acceleration
...
class LorenzDataset:
    ...
    def getitem_past_future_x(self, t, K):
        # Return window from t to t+K-1
        return self.xyz[t-K:t, 0], self.xyz[t:t+K, 0]
...
if __name__ == '__main__':
    ...
    past_sample, future_sample = data.getitem_past_future_x(t=20, K=10)

    model = LorentzPredictor(input_size=10, output_size=10)
    ...
    # Training loop random window size K
    # batch_size = 16
    # for epoch in range(1000):
    #
    #         t_rnd = np.random.randint(window_size, data.len()-window_size)
    #
    #         mse_loss = 0
    #         for n in range(batch_size):
    #             past_sample, future_sample = data.getitem_past_future_x(t=t_rnd, K=window_size)
    #             pred = model(past_sample)
    #             mse_loss += F.mse_loss(pred, future_sample)
    #
    #         optimizer.zero_grad()
    #         mse_loss.backward()
    #         optimizer.step()
    #
    #         print(f'Epoch {epoch}, loss {mse_loss.item()/batch_size}')
    ...
    window_size = 10
    step = data.len()//window_size
    iteration = 1000

    for epoch in range(1000):

            mse_loss = 0
            pred, xxx = data.getitem_past_future_x(t=10, K=10)

            for n in range(iteration):
                xxx, future_sample = data.getitem_past_future_x(t=n, K=10)
                pred = model(pred)
                mse_loss += F.mse_loss(pred, future_sample)
    ...
    past_sample, future_sample = data.getitem_past_future_x(t=0, K=10)
    pred = model(pred)

    print (f'past: {to_numpy(past_sample)}')
    print (f'true future: {to_numpy(future_sample)}')
    print (f'predicted future: {to_numpy(pred)}')
