...
if __name__ == '__main__':
    ...
    if args.option!=None:
        ...
        config_list = ['falling_water_ramp_x41', 'falling_water_ramp_x42', 'falling_water_ramp_x43', 'falling_water_ramp_x44', 'falling_water_ramp_x45', 'falling_water_ramp_x46', 'falling_water_ramp_x47']
    ...
    for config_file in config_list:
        ...
        if 'test' in action:
            data_test(config=config, config_file=config_file, visualize=True, style='black color arrow speed acc_learned boundary', verbose=False, best_model='best', run=2, plot_data=False,
                      test_simulation=False, sample_embedding=False, device=device, fixed=True, bounce=True, step=1) # config.simulation.n_frames // 200, )  arrow speed acc_learned   arrow speed acc_true
