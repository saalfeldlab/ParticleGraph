...
def data_generate(model_config, bVisu=True, bDetails=False, bSave=True, step=5):
    ...
    for run in range(model_config['nrun']):
        ...
        if (model_config['model'] == 'WaveMesh') | (model_config['boundary'] == 'periodic'):
            ...
            values = i0[
                (X1[:, 0].detach().cpu().numpy() * 255).astype(int), (X1[:, 1].detach().cpu().numpy() * 255).astype(
                    int)]
        ...
        for it in tqdm(range(model_config['start_frame'], nframes)):
            ...
            if (run == 0) & (it % step == 0) & (it >= 0) & bVisu:
                ...
                elif (model_config['model'] == 'HeatParticles') | (model_config['model'] == 'DiffMesh') | (
                        model_config['model'] == 'WaveMesh'):
                    plt.scatter(x_noise[:, 1].detach().cpu().numpy(), x_noise[:, 2].detach().cpu().numpy(), s=10,
                                alpha=0.75,
                ...
                if bDetails:
                    ...
                    if model_config['model'] == 'GravityParticles':
                        for n in range(nparticle_types):
                            ...
                            model_config['model'] == 'WaveMesh'):
                        plt.scatter(x_noise[:, 1].detach().cpu().numpy(), x_noise[:, 2].detach().cpu().numpy(), s=60,
                                    alpha=0.75,
                                    c=x[:, 6].detach().cpu().numpy(), cmap='gist_gray', vmin=-5000, vmax=5000)
                        ax.set_facecolor([0.5, 0.5, 0.5])
                    elif model_config['model'] == 'ElecParticles':
...
def data_train(model_config, bSparse=False):
    ...
    # net = f"./log/try_{ntry}/models/best_model_with_9_graphs_14.pt"
    ...
    for epoch in range(Nepochs + 1):
        ...
        if (total_loss / nparticles / batch_size / N < best_loss):
            ...
            print(
                "Epoch {}. Loss: {:.6f} sparsity_index {:.3f} ".format(epoch, total_loss / N / nparticles / batch_size,
                                                                       sparsity_index.item()))
        ...
        if bSparse:
            ...
            if model_config['model'] == 'ElecParticles':
                        ...
                        for n in index_particles[k]:
                            ...
                            if n % 5 == 0:
                                plt.plot(rr.detach().cpu().numpy(),
                                         acc.detach().cpu().numpy() * ynorm[4].detach().cpu().numpy() / model_config[
                                             'tau'], linewidth=1,
                ...
                for n in range(nparticle_types):
                    plt.plot(kmeans.cluster_centers_[n, 0], kmeans.cluster_centers_[n, 1], '+', color='k',
                             markersize=12)
                ...
                for n in range(nparticles):
                    ...
                    if model_config['prediction'] == '2nd_derivative':
                        ...
                        if model_config['prediction'] == 'first_derivative_L':
                            in_features = torch.cat((-rr[:, None] / model_config['radius'], 0 * rr[:, None],
                                                     rr[:, None] / model_config['radius'], 0 * rr[:, None],
                                                     0 * rr[:, None], 0 * rr[:, None], 0 * rr[:, None], embedding),
                                                    dim=1)
...
def data_test(model_config, bVisu=False, bPrint=True, index_particles=0, prev_nparticles=0, new_nparticles=0,
              prev_index_particles=0, best_model=0, step=5, bTest='', folder_out='tmp_recons'):
    ...
    if new_nparticles > 0:  # nparticles larger than initially
        ...
        if ratio_particles > 1:
            ...
            model.a = nn.Parameter(
                torch.tensor(
                    np.ones((model.ndataset, int(prev_nparticles) * ratio_particles, model_config['embedding'])),
            ...
            for k in range(5):
                ...
                if ((k == 1) | (k == 2)) & ('Boids' in model_config['description']):
                    for n, boid in enumerate(flock):
                        ...
                        ps = np.array(ps)
                        plt.plot(ps[:, 0], ps[:, 1], c=cmap.color(T1[n].detach().cpu().numpy()), alpha=0.5)
                elif (k == 0) & ((model_config['model'] == 'DiffMesh') | (model_config['model'] == 'WaveMesh') | (
                        model_config['model'] == 'HeatParticles')):
                    plt.scatter(x0_next[:, 6].detach().cpu().numpy(), x[:, 6].detach().cpu().numpy(), s=1, alpha=0.25,
                                cmap='gist_gray', vmin=-5000, vmax=5000, c='k')
                ...
                if (k > 2) & (model_config['model'] != 'HeatParticles') & (model_config['model'] != 'DiffMesh') & (
                        model_config['model'] != 'WaveMesh'):
                    for n in range(nparticles):
...
def data_test_generate(model_config, bVisu=True, bDetails=False, step=5):
    ...
    values = i0[
        (X1[:, 0].detach().cpu().numpy() * 256).astype(int), (X1[:, 1].detach().cpu().numpy() * 256).astype(int)]
...
def data_plot(model_config, epoch, bPrint, best_model=0):
    ...
    if model_config['model'] == 'ElecParticles':
        ...
        proj_interaction = trans.transform(coeff_norm)
        proj_interaction = np.squeeze(proj_interaction)
        particle_types = x_stat[0][0, :, 5].clone().detach().cpu().numpy()
        ax = fig.add_subplot(2, 4, 4)
        for n in range(nparticle_types):
            plt.scatter(proj_interaction[index_particles[n], 0], proj_interaction[index_particles[n], 1], s=5)
        plt.xlabel('UMAP 0', fontsize=12)
...
def load_model_config(id=48):
    ...
    if id == 77:
        model_config_test = {'ntry': id,
                             ...
                             'prediction': 'first_derivative_S',
                             'upgrade_type': 'none',
                             'p': [[1.2425, 1.3355, 1.3397, 1.3929], [1.629, 1.4932, 1.5311, 1.8677],
                                   [1.9852, 1.1892, 1.1544, 1.993], [1.6898, 1.1336, 1.4869, 1.7767],
                                   [1.8847, 1.5448, 1.8063, 1.3873], [1.496, 1.4064, 1.9045, 1.733],
                                   [1.5108, 1.9904, 1.1665, 1.6975], [1.6153, 1.8557, 1.2758, 1.0684]],
                             'nrun': 2,
                             'start_frame': -20,
                             'arrow_length': 20,
                             'cmap': 'tab10',
    ...
    if id == 142:
        model_config_test = {'ntry': id,
                             ...
                             # separation alignement cohesion
                             'nrun': 10,
                             'clamp': 1E-3,
                             'pred_limit': 1E9,
                             'start_frame': 0.,
    ...
    if id == 144:
        model_config_test = {'ntry': id,
                             ...
                             'nlayers_update': 3,
                             'hidden_size_update': 64,
                             'p': [[50, 10, 0], [10, 30, 40], [20, 50, 40], [35, 80, 40]],
                             # separation alignement cohesion
                             'nrun': 10,
                             'clamp': 1E-3,
                             'pred_limit': 1E9,
