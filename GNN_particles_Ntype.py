            ...
            if (run == 0) & (it % 5 == 0):
                ...
                if model_config['model'] == 'GravityParticles':
                    ...
                    for n in range(nparticle_types):
                        ...
                        if n == 0:
                            plt.scatter(X1t[index_particles[n], 0, it], X1t[index_particles[n], 1, it], s=g,
                                        c='r')  # , facecolors='none', edgecolors='k')
                        else:
                            plt.scatter(X1t[index_particles[n], 0, it], X1t[index_particles[n], 1, it], s=g,
                                        c='b')  # , facecolors='none', edgecolors='k')
...
def data_train(model_config, gtest):
    ...
    net = f"./log/try_50/models/best_model_with_1_graphs.pt"
...
def data_plot(model_config):
    ...
    if model_config['model'] == 'GravityParticles':
        ...
        tmean = np.ones(kl_peaks)
        tstd = np.ones(kl_peaks)
        new_index_particles=[]
        for n in range(kl_peaks):
            # pos=np.argwhere(kmeans_.labels_==n).astype(int)
            # new_index_particles.append(pos)
            plt.hist(t[index_particles[n]].squeeze(),25)
...
def load_model_config(id=48):
    ...
    if id == 56:
        model_config_test = {'ntry': id,
                             ...
                             'clamp': 0.002,
                             'pred_limit': 1E9}
    # gravity MLPs from 50 (3 masses 960 particles) learn embedding from 54 (8 masses 960 particles)
    if id == 57:
        model_config_test = {'ntry': id,
                                 'input_size': 8,
                                 'output_size': 2,
                                 ...
                                 'clamp': 0.002,
                                 'pred_limit': 1E9}
    # gravity MLPs from 50 (3 masses 960 particles) learn embedding from 55 (20 masses 960 particles)
    if id == 58:
        model_config_test = {'ntry': id,
                                 'input_size': 8,
                                 'output_size': 2,
                                 ...
                                 'clamp': 0.002,
                                 'pred_limit': 1E9}
    # gravity MLPs from 50 (3 masses 960 particles) learn embedding from 56 (30 masses 960 particles)
    if id == 59:
        model_config_test = {'ntry': id,
                                 'input_size': 8,
                                 'output_size': 2,
...
if __name__ == '__main__':
    ...
    for gtest in range(57,60): #(57,60):
