        ...
        if (it % step == 0) & (it>=0) &  bVisu:
            ...
            for k in range(5):
                ...
                if ((k==1)|(k==2))&('Boids' in model_config['description']):
                    ...
                    if model_config['model'] == 'WaveMesh':
                        plt.tripcolor(pts[:, 0], pts[:, 1], tri.simplices.copy(), facecolors=colors.detach().cpu().numpy(),edgecolors='k')
                    else:
                        plt.tripcolor(pts[:, 0], pts[:, 1], tri.simplices.copy(),
...
if __name__ == '__main__':
    ...
    for gtest in gtestlist:
        ...
        x, rmserr_list = data_test(model_config, bVisu=True, bPrint=True, best_model=18, step=5, bTest='', initial_map='')
