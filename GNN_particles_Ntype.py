                    ...
                    if bDetails:
                        ...
                        if model_config['model'] == 'GravityParticles':
                            ...
                            if model_config['model'] == 'WaveMesh':
                                ...
                                plt.tripcolor(pts[:, 0], pts[:, 1], tri.simplices.copy(),
                                              facecolors=colors.detach().cpu().numpy(), edgecolors='k', vmin=0,
                                              vmax=2500)
                            # ax.set_facecolor([0.5,0.5,0.5])
                        elif model_config['model'] == 'ElecParticles':
                            for n in range(nparticle_types):
...
def data_plot(model_config, epoch, bPrint, best_model=0, kmeans_input='plot'):
        ...
        for m in range(nparticle_types):
            for n in range(nparticle_types):
                if plot_list_pairwise[nn][10] < 0:
                    popt, pocv = curve_fit(func_pow, rr.detach().cpu().numpy(),
                                           -plot_list_pairwise[nn].detach().cpu().numpy(), bounds=([0, 1.5], [5., 2.5]))
                    popt[0] = -popt[0]
                else:
                    popt, pocv = curve_fit(func_pow, rr.detach().cpu().numpy(),
                                           plot_list_pairwise[nn].detach().cpu().numpy(), bounds=([0, 1.5], [5., 2.5]))
    ...
    if model_config['model'] == 'GravityParticles':
        ...
        for n in range(len(r_list)):
            plt.plot(vv.detach().cpu().numpy(), plot_list_2[n].detach().cpu().numpy(), linewidth=1, color=cmap.color(n),
                     label=f'r={r_list[n]}')
...
def data_train_shrofflab_celegans(model_config):
    ...
    logger.info(
        f'xnorm vnorm ynorm: {xnorm.detach().cpu().numpy(), vnorm.detach().cpu().numpy(), ynorm.detach().cpu().numpy()}')
...
def data_test_shrofflab_celegans(model_config):
    ...
    for k in tqdm(range(nframes[run] - 2)):
        ...
        ax.scatter(x[:, 1].detach().cpu().numpy(), x[:, 3].detach().cpu().numpy(), x[:, 2].detach().cpu().numpy(),
                   c=embedding[:, 1].detach().cpu().numpy(), alpha=1, vmin=mina, vmax=maxa)
        ax.set_aspect('equal')
        # remove axis ticks
        ax.set_xticks([])
        ax.set_yticks([])
