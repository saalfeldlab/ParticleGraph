            ...
            for step in range(0,3):
                ...
                if step == 4:
                    ...
                    for grid in range(1):
                        ...
                        X1 = torch.rand(nparticles, 2, device=device, dtype=torch.float32)
                        V1 = torch.zeros((nparticles, 2), device=device, dtype=torch.float32,)

                        x = torch.tensor(np.zeros((int(nparticles), 4)), dtype=torch.float32, device=device, requires_grad=True)
                        x.data[:,0:2] = X1
                        x.data[:,2:4]= V1
                        ...
                        x0 = torch.load(f'graphs_data/graphs_particles_{datum}/x_0_{nframes}.pt')
                        x0 = x0.to(device)
                        ...
                        for it in range(10000):
                            ...
                            dataset = data.Data(x=torch.cat((x,x0[:,4:7]),axis=1), edge_index=edge_index)
                            ...
                            if (it % stp == 0):
                                ...
                                ax = fig.add_subplot(2, 3, 5)
                                for n in range(nparticle_types):
                                    plt.scatter(y[index_particles[n], 0].detach().cpu(), y[index_particles[n], 1].detach().cpu(), s=3)
                                ax = plt.gca()
                                ax.axes.xaxis.set_ticklabels([])
                                ax.axes.yaxis.set_ticklabels([])
                                plt.xlim([-0.3, 1.3])
    ...
    for gtest in range(1):
        ...
        for n in range(nparticle_types):
            index_particles.append(
                np.arange(int(nparticles / nparticle_types) * n, int(nparticles / nparticle_types) * (n + 1)))
        ...
        for step in range(1, 3):
            ...
            if step == 0:
                ...
                for n in range(nparticle_types):
                    model.append(InteractionParticles_0(aggr_type=aggr_type, p=torch.squeeze(p[n]), tau=tau))
                    torch.save({'model_state_dict': model[n].state_dict()},
                               f'graphs_data/graphs_particles_{datum}/model_{n}.pt')
                    psi_output.append(psi(rr, torch.squeeze(p[n])))
                    print(f'p{n}: {np.round(torch.squeeze(p[n]).detach().cpu().numpy(), 4)}')
                time.sleep(0.5)
                ...
                for gridsearch in gridsearch_list:
                    ...
                    for epoch in range(60):
                        ...
                        if (total_loss / N / nparticles < best_loss):
                            ...
                            print("Epoch {}. Loss: {:.6f} Gap: {:.3f} kl.elbow {:d} ".format(epoch,
                                                                                             total_loss / N / nparticles,
                                                                                             gap, kl.elbow))

                        if (((gap < 100) & (kl.elbow == nparticle_types)) | ((epoch > 49))) & (
            ...
            if step == 2:
                ...
                for it in tqdm(range(nframes - 1)):
                    ...
                    if (it % stp == 0):
                        ...
                        ax = fig.add_subplot(2, 3, 1)
                        for n in range(nparticle_types):
                            plt.scatter(x00[index_particles[n], 0].detach().cpu(),
                                        x00[index_particles[n], 1].detach().cpu(), s=3)
            ...
            if step == 3:
                ...
                for it in range(nframes, 0, -1):
                    ...
                    for loop in range(10000):
                        ...
                        loss1 = (x_p - target_t_1[:, 0:2]).norm(2) + (v_p - target_t_1[:, 2:4]).norm(
                            2)  # + 10*(torch.std(v_p) - torch.std(target[:,2:4])).norm(2) + 10*(torch.mean(v_p) - torch.mean(target[:,2:4])).norm(2)
                        ...
                        loss2 = (x_pp - target_t[:, 0:2]).norm(2) + (v_pp - target_t[:, 2:4]).norm(
                            2)  # + 10*(torch.std(v_p) - torch.std(target[:,2:4])).norm(2) + 10*(torch.mean(v_p) - torch.mean(target[:,2:4])).norm(2)
                    ...
                    if (it % stp == 0):
                        ...
                        ax = fig.add_subplot(2, 3, 5)
                        for n in range(nparticle_types):
                            plt.scatter(xx[index_particles[n], 0].detach().cpu(), xx[index_particles[n], 1].detach().cpu(),
                                        s=3)
                        ax = plt.gca()
                        ax.axes.xaxis.set_ticklabels([])
                        ax.axes.yaxis.set_ticklabels([])
            ...
            if step == 4:
                ...
                for grid in range(1):
                    ...
                    for it in range(10000):
                        ...
                        loss = 0
                        for n in range(nparticle_types):
                            loss += 1E1 * (y[index_particles[n], 0:2] - x0[index_particles[n], 0:2]).norm(2) + (
                                        y[index_particles[n], 2:4] - x0[index_particles[n], 2:4]).norm(2)
                            loss += g_loss(y[index_particles[n], 0:4], x0[index_particles[n], 0:4]) * 1E2
                            loss += g_loss(y[index_particles[n], 0:4], x0[index_particles[n], 0:4]) * 1E4
                        ...
                        if (it % stp == 0):
                            ...
                            ax = fig.add_subplot(2, 3, 2)
                            for n in range(nparticle_types):
                                plt.scatter(x0[index_particles[n], 0].detach().cpu(),
                                            x0[index_particles[n], 1].detach().cpu(), s=3)
                            ax = plt.gca()
                            plt.xlim([-0.3, 1.3])
                            plt.ylim([-0.3, 1.3])
                            ax.axes.get_xaxis().set_visible(False)
                            ...
                            ax = fig.add_subplot(2, 3, 4)
                            for n in range(nparticle_types):
                                plt.scatter(x[index_particles[n], 0].detach().cpu(),
                                            x[index_particles[n], 1].detach().cpu(), s=3)
                            ax = plt.gca()
                            ax.axes.xaxis.set_ticklabels([])
                            ax.axes.yaxis.set_ticklabels([])
                            plt.xlim([-0.3, 1.3])
                            ...
                            ax = fig.add_subplot(2, 3, 5)
                            for n in range(nparticle_types):
                                plt.scatter(y[index_particles[n], 0].detach().cpu(),
                                            y[index_particles[n], 1].detach().cpu(), s=3)
                            ax = plt.gca()
                            ax.axes.xaxis.set_ticklabels([])
                            ax.axes.yaxis.set_ticklabels([])
                            plt.xlim([-0.3, 1.3])
    ...
    for gtest in range(1):
        ...
        for step in range(1, 3):
                ...
                for run in tqdm(range(gridsearch_list[0] + 1)):
                    ...
                    for it in range(nframes):
                        ...
                        if (run == 0) & (it % 5 == 0):
                            ...
                            pos = dict(enumerate(x[:, 0:2].detach().cpu().numpy(), 0))
                            vis = to_networkx(dataset2, remove_self_loops=True, to_undirected=True)
                            nx.draw_networkx(vis, pos=pos, node_size=10, linewidths=0, edge_color='b',
                                             with_labels=False)
                            plt.xlim([-0.3, 1.3])
                            plt.ylim([-0.3, 1.3])
                            plt.text(-0.25, 1.33, f'Graph    {x.shape[0]} nodes {edge_index.shape[1]} edges ',
                                     fontsize=10)
                            ...
                            plt.text(-0.25, 1.33, f'sigma:{sigma} N:{nparticles} nframes:{nframes}')
                            for n in range(nparticle_types):
                                plt.text(-0.25, 1.25 - n * 0.05,
                                         f'p{n}: {np.round(model[n].p.detach().cpu().numpy(), 4)}',
                                         color='k')
                ...
                for gridsearch in gridsearch_list:
                    ...
                    for epoch in range(60):
                        ...
                        for N in range(1, (gridsearch - 1) * nframes * data_augmentation_loop, stp):
                            ...
                            if data_augmentation:
                                phi = torch.randn(1, dtype=torch.float32, requires_grad=False,
                        ...
                        for n in range(nparticle_types):
                            embedding_particle.append(embedding[index_particles[n], :])
                        kmeans = KMeans(init="random", n_clusters=nparticle_types, n_init=10, max_iter=300,
                                        random_state=42)
                        kmeans.fit(embedding)
                        gap = kmeans.inertia_
                        kmeans_kwargs = {"init": "random", "n_init": 10, "max_iter": 300, "random_state": 42}
            ...
            if step == 2:
                ...
                for it in tqdm(range(nframes - 1)):
                    ...
                    if (it % stp == 0):
                        ...
                        for n in range(nparticle_types):
                            plt.scatter(x0[index_particles[n], 0].detach().cpu(),
                                        x0[index_particles[n], 1].detach().cpu(),
                                        s=3)
                        ax = plt.gca()
                        plt.xlim([-0.3, 1.3])
                        plt.ylim([-0.3, 1.3])
                        ...
                        for n in range(nparticle_types):
                            plt.scatter(x[index_particles[n], 0].detach().cpu(),
                                        x[index_particles[n], 1].detach().cpu(),
                                        s=3)
                        ax = plt.gca()
                        ax.axes.xaxis.set_ticklabels([])
                        ax.axes.yaxis.set_ticklabels([])
            ...
            if step == 3:
                ...
                xx = torch.tensor(np.zeros((int(nparticles), 4)), dtype=torch.float32, device=device,
                                  requires_grad=True)
                xx.data[:, 0:4] = x[:, 0:4]
                ...
                for it in range(nframes, 0, -1):
                    ...
                    if (it % stp == 0):
                        ...
                        ax = fig.add_subplot(2, 3, 5)
                        for n in range(nparticle_types):
                            plt.scatter(xx[index_particles[n], 0].detach().cpu(),
                                        xx[index_particles[n], 1].detach().cpu(),
                                        s=3)
                        ax = plt.gca()
                        ax.axes.xaxis.set_ticklabels([])
                        ax.axes.yaxis.set_ticklabels([])
            ...
            if step == 4:
                ...
                for grid in range(1):
                    ...
                    x = torch.tensor(np.zeros((int(nparticles), 4)), dtype=torch.float32, device=device,
                                     requires_grad=True)
                    x.data[:, 0:2] = X1
                    x.data[:, 2:4] = V1
