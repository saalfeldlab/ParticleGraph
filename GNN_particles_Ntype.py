...
if __name__ == '__main__':
    ...
    for config_file in config_list:
        ...
        # data_generate(config, device=device, visualize=True, run_vizualized=0, style='color cell_id', alpha=1, erase=True, bSave=True, step=1) # config.simulation.n_frames // config.simulation.n_frames)
        # data_train(config, config_file, device)
        data_test(config=config, config_file=config_file, visualize=True, style='frame color', verbose=False, best_model=0, run=1, step=1, save_velocity=True) #config.simulation.n_frames // 3, test_simulation=False, sample_embedding=False, device=device)    # config.simulation.n_frames // 7
