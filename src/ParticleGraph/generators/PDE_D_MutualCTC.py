...
class PDE_D_MutualCTC(pyg.nn.MessagePassing):
    """
    Mutualistic CTC: complementary particle-field coupling for 2-type convergence.

    The key insight is that the 2-type convergence barrier arises because BOTH types
    deplete C1 (consume it), causing the field to collapse below the CTC threshold
    wherever particles aggregate. This creates a self-defeating feedback: particles
    move toward the CTC isoline, consume C1, lower it, and must chase the receding
    isoline — preventing convergence.

    MutualCTC breaks this by making types COMPLEMENTARY:
    - Type 0: CONSUMES C1 (sink) — pulls C1 down toward threshold from above
    - Type 1: PRODUCES C1 (source) — pushes C1 up toward threshold from below
    When both types coexist near the CTC isoline, their opposing effects on C1
    STABILIZE the field at the threshold. This creates a self-reinforcing equilibrium:
    the field stays at T because sources and sinks balance.

    The mutualistic coupling strength is parameterized by mutual_strength:
    - Type 0 pf: dC1 = -consumption * w(r) * mutual_strength
    - Type 1 pf: dC1 = +production_boost * w(r) * mutual_strength
    The net pf effect depends on local type composition, creating LOCAL field
    homeostasis wherever both types aggregate together.

    Physics:
    1. fp: Durotaxis + CTC (same as DampedCTC) — drives particles to isoline
    2. pf: COMPLEMENTARY consumption/production — Type 0 sinks, Type 1 sources
    3. pp: Field-damped attraction-repulsion (Painter & Hillen 2002)

    Literature:
    - Turing, A. M. (1952) Phil Trans R Soc Lond B 237:37-72
      "The chemical basis of morphogenesis" — activator-inhibitor principle
    - Kondo, S. & Miura, T. (2010) Science 329:1616-1620
    ...
    NOTE: For MutualCTC, Type 1's consumption should be NEGATIVE (i.e., production of C1).
    """

    PARAMS_DOC = {
        "model_name": "MutualCTC",
        "literature": "Turing (1952); Kondo & Miura (2010); Wolpert (1969); Painter & Hillen (2002)",
        "description": "Mutualistic CTC with complementary particle-field coupling for 2-type convergence",
        "equations": {
            "field_to_particle": "v = M*(1+alpha*|gradC1|)*(-tanh(3*(C1-T)/A))*grad*dir / (1 + fp_drag*|vel|/v_ref)",
            "particle_to_field": "Type 0: dC1 = -consumption*w(r), Type 1: dC1 = +production_boost*w(r) [COMPLEMENTARY]",
        ...
        "params_mesh": [
            ...
            },
            {
                ...
                "slots": [
                    ...
                    {"index": 6, "name": "mutual_strength", "description": "Mutualistic coupling amplification (1.0=normal, 2.0=2x)", "typical_range": [0.5, 3.0]},
        ...
        "particle_params": {
            "description": "Per-type params from simulation.params (one row per n_particle_types). NOTE: Type 1 consumption should be NEGATIVE for mutualistic coupling!",
            "slots": [
                {"index": 0, "name": "M1", "description": "Per-type mobility for C1"},
                {"index": 1, "name": "M2", "description": "Per-type mobility for C2"},
                {"index": 2, "name": "consumption", "description": "Per-type consumption rate (NEGATIVE = production!)"},
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_MutualCTC, self).__init__(aggr=aggr_type)
        ...
        self.mutual_strength = p[2, 6] if p.shape[1] > 6 else 1.0

        print(f"initialized PDE_D_MutualCTC with parameters:")
        ...
        ms_val = self.mutual_strength.item() if hasattr(self.mutual_strength, 'item') else self.mutual_strength
        print(f"  mutual_strength={ms_val:.3f} (Turing 1952; Kondo & Miura 2010)")
        ...
        if particle_params is not None:
            n_types = particle_params.shape[0]
            for t in range(n_types):
                cons_val = particle_params[t, 2].item()
                prod_val = particle_params[t, 3].item()
                role = "SINK (consumes C1)" if cons_val > 0 else "SOURCE (produces C1)"
                print(f"    Type {t}: consumption={cons_val:.1f}, production={prod_val:.1f} => {role}")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_MutualCTC: particle_params has {n_param_rows} rows but found "
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            if parameters_i is not None:
                # Per-type consumption/production — the KEY mutualistic mechanism
                # Type 0 should have positive consumption (sink), Type 1 negative (source)
                consumption = parameters_i[:, 2] * self.mutual_strength
                production = parameters_i[:, 3]
            else:
                consumption = self.consumption_rate * self.mutual_strength
