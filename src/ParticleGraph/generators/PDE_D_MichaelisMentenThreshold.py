...
class PDE_D_MichaelisMentenThreshold(pyg.nn.MessagePassing):
    """
    Hybrid particle model combining Michaelis-Menten saturating kinetics with
    concentration-threshold bistable coupling (CTC).

    This model combines two mechanisms:
    1. Michaelis-Menten (Michaelis & Menten 1913): Consumption/production rates
       saturate at high field concentrations:
       rate_eff = base_rate * |C1| / (Km + |C1|)
    ...
    The combination enables self-limiting consumption/production dynamics (from
    Michaelis-Menten saturation) while maintaining convergent stability (from
    threshold coupling). The MM kinetics prevent runaway consumption at high
    concentrations, potentially preserving richer morphology than linear models.
    ...
    PARAMS_DOC = {
        "model_name": "MichaelisMentenThreshold",
        "literature": "Michaelis & Menten (1913) Biochem Z 49:333; Wolpert (1969) J Theor Biol 25:1",
        "description": "Saturating kinetics + bistable concentration-threshold coupling",
        "equations": {
            "field_to_particle": "v = (M1*grad_C1 + M2*grad_C2) * (-tanh(3*(C1-T)/A)) * dir",
        ...
        "params_mesh": [
            {
                ...
                "slots": [
                    ...
                    {"index": 6, "name": "mm_Km", "description": "Michaelis-Menten half-saturation constant (0=constant rate)", "typical_range": [0.1, 5.0]},
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_MichaelisMentenThreshold, self).__init__(aggr=aggr_type)
        ...
        self.mm_Km = p[0, 6] if p.shape[1] > 6 else 0.0
        ...
        print(f"initialized PDE_D_MichaelisMentenThreshold with parameters:")
        print(f"  mobility: M1={self.M1.item()}, M2={self.M2.item()}")
        mm_val = self.mm_Km.item() if hasattr(self.mm_Km, 'item') else self.mm_Km
        print(f"  mm_Km={mm_val:.3f} (rate = base * |C1|/(Km+|C1|), Michaelis-Menten 1913)")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_MichaelisMentenThreshold: particle_params has {n_param_rows} rows but found "
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            # Michaelis-Menten modulation of consumption/production (Michaelis & Menten 1913)
            if self.mm_Km > 0:
