...
class PDE_D_DragDampedCTC(pyg.nn.MessagePassing):
    """
    Durotaxis + CTC with field-dependent pp damping AND velocity-dependent fp drag.

    Extends DampedCTC by adding a velocity-dependent attenuation to the
    field-to-particle (fp) chemotactic response. When particles move fast
    (high |v|), the fp driving force is reduced by a Stokes-like drag factor:
        fp_damped = fp / (1 + fp_drag * |v| / v_ref)
    This prevents particles from overshooting the CTC threshold isoline,
    targeting the convergence bottleneck where fast-moving particles oscillate
    past their target concentration.

    Physical motivation: receptor binding kinetics — a fast-moving cell has
    less time to sample local concentration gradients, reducing effective
    chemotactic sensitivity (Tranquillo & Lauffenburger 1987).

    Physics:
    1. fp: Durotaxis + CTC + velocity drag
    ...
    PARAMS_DOC = {
        "model_name": "DragDampedCTC",
        "literature": "Lo (2000); Wolpert (1969); Painter & Hillen (2002); Tranquillo & Lauffenburger (1987)",
        "description": "Durotaxis + CTC + pp damping + velocity-dependent fp drag",
        "equations": {
        ...
        "params_mesh": [
            ...
            },
            {
                ...
                "slots": [
                    ...
                    {"index": 4, "name": "fp_drag", "description": "Velocity-dependent fp drag (0=off, >0 reduces fp at high velocity)", "typical_range": [0.0, 5.0]},
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_DragDampedCTC, self).__init__(aggr=aggr_type)
        ...
        self.v_ref = 0.01  # reference velocity scale for normalization

        print(f"initialized PDE_D_DragDampedCTC with parameters:")
        ...
        print(f"  fp_drag={fp_drag_val:.3f}, v_ref={self.v_ref:.4f} (Tranquillo & Lauffenburger 1987)")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_DragDampedCTC: particle_params has {n_param_rows} rows but found "
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            # When particle is moving fast, reduce fp response to prevent overshoot
            if self.fp_drag > 0:
                vel_i = x_i[:, 1+self.dimension:1+2*self.dimension]  # velocity columns
