...
def choose_mesh_model(model_config, device):
    model_name = model_config['model']
    n_node_types = model_config['nnode_types']
    aggr_type = model_config['mesh_aggr_type']
    _, bc_dpos = choose_boundary_values(model_config['boundary'])
    ...
    match model_name:
        case 'RD_Gray_Scott_Mesh':
            mesh_model = RD_Gray_Scott(aggr_type=aggr_type, c=torch.squeeze(c), beta=model_config['beta'], bc_dpos=bc_dpos)
        case 'RD_FitzHugh_Nagumo_Mesh':
            mesh_model = RD_FitzHugh_Nagumo(aggr_type=aggr_type, c=torch.squeeze(c), beta=model_config['beta'], bc_dpos=bc_dpos)
        case 'RD_RPS_Mesh':
            mesh_model = RD_RPS(aggr_type=aggr_type, c=torch.squeeze(c), beta=model_config['beta'], bc_dpos=bc_dpos)
        case 'DiffMesh' | 'WaveMesh':
            mesh_model = Laplacian_A(aggr_type=aggr_type, c=torch.squeeze(c), beta=model_config['beta'], bc_dpos=bc_dpos)
        case 'Maze':
            c = initialize_random_values(n_node_types, device)
            for n in range(n_node_types):
                c[n] = torch.tensor(model_config['c'][n])
