...
class PDE_D_CrossTypeAdhesionThreshold(pyg.nn.MessagePassing):
    """
    Hybrid particle model combining Steinberg differential adhesion with
    concentration-threshold bistable coupling.

    This model combines two mechanisms:
    1. ThresholdCoupling (Wolpert 1969): Bistable response reverses mobility
       sign at concentration threshold T, creating attractor at C1=T isoline.
       sign_factor = -tanh(steepness * (C1 - T) / A)
    2. CrossTypeAdhesion (Steinberg 1963): Same-type particles attract normally;
       cross-type particles experience inverted forces, driving cell sorting.

    The combination enables rich multi-domain morphology (from differential
    adhesion sorting) while maintaining convergent stability (from threshold
    coupling). Particles segregate by type via adhesion AND lock onto
    concentration isolines via threshold coupling.
    ...
    PARAMS_DOC = {
        "model_name": "CrossTypeAdhesionThreshold",
        "literature": "Steinberg (1963) Science 141:401; Wolpert (1969) J Theor Biol 25:1",
        "description": "Differential adhesion sorting + bistable concentration-threshold coupling",
        "equations": {
        ...
        "params_mesh": [
            ...
            },
            {
                ...
                "slots": [
                    ...
                    {"index": 5, "name": "cross_type_factor", "description": "Cross-type adhesion inversion AND per-type CTC threshold spread", "typical_range": [0.1, 0.5]}
                ]
            }
        ],
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_CrossTypeAdhesionThreshold, self).__init__(aggr=aggr_type)
        ...
        # Cross-type factor: controls BOTH adhesion inversion AND per-type CTC threshold spread
        self.cross_type_factor = p[2, 5] if p.shape[1] > 5 else 0.0

        print(f"initialized PDE_D_CrossTypeAdhesionThreshold with parameters:")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_CrossTypeAdhesionThreshold: particle_params has {n_param_rows} rows but found "
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            if parameters_i is not None:
                ...
                # Cross-type differential adhesion (Steinberg 1963):
