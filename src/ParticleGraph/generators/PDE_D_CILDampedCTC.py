...
class PDE_D_CILDampedCTC(pyg.nn.MessagePassing):
    """
    CIL + CTC + pp damping: combining contact inhibition of locomotion with
    concentration-threshold coupling and field-dependent pp damping.

    Hypothesis: For 1-type particles, CTC alone fails because without a neutral
    buffer type, particles oscillate past the threshold isoline. CIL provides
    density-dependent slowdown that acts as a substitute: particles at high local
    density reduce their chemotactic response, preventing overshoot. Combined with
    pp damping at the CTC threshold, this should enable 1-type convergence beyond
    the CIL-only ceiling of 0.4197.

    Physics:
    1. fp: CIL-modulated CTC coupling
       v = M * f(rho) * (1+alpha*|gradC1|) * (-tanh(steep*(C1-T)/A)) * grad * dir
       where f(rho) = 1 / (1 + (rho/rho_0)^n) is the Hill function density brake
    ...
    Literature:
    - Mayor & Carmona-Fontaine (2010) Trends Cell Biol 20:319-328
    ...
    PARAMS_DOC = {
        "model_name": "CILDampedCTC",
        "literature": "Mayor (2010) TCB 20:319; Wolpert (1969) JTB 25:1; Painter & Hillen (2002) CAMQ 10:501; Lo (2000) Biophys J 79:144",
        "description": "CIL density-dependent mobility + CTC threshold + pp damping. Designed for 1-type convergence.",
        "equations": {
            "field_to_particle": "v = M * f(rho) * (1+alpha*|gradC1|) * (-tanh(3*(C1-T)/A)) * grad * dir",
        ...
        "params_mesh": [
            ...
            {
                "row": 1, "description": "C2 field parameters + pp damping + CIL",
                "slots": [
                    ...
                    {"index": 3, "name": "pp_damping_width", "description": "Width of pp damping zone (in units of A)", "typical_range": [0.1, 1.0]},
                    {"index": 4, "name": "rho_0", "description": "CIL critical density (0=auto)", "typical_range": [10, 50]},
                    {"index": 5, "name": "hill_n", "description": "CIL Hill coefficient (0=default 2)", "typical_range": [1, 4]},
                    {"index": 6, "name": "unused1", "description": "Pad"},
                    {"index": 7, "name": "unused2", "description": "Pad"}
                ]
            },
            {
                "row": 2, "description": "Particle-field coupling + cross-type threshold spread",
                "slots": [
                    ...
                    {"index": 4, "name": "unused3", "description": "Pad"},
                    {"index": 5, "name": "cross_type_factor", "description": "Per-type CTC threshold spread", "typical_range": [0.0, 0.5]},
                    {"index": 6, "name": "unused4", "description": "Pad"},
                    {"index": 7, "name": "unused5", "description": "Pad"}
                ]
            }
        ]
    }
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_CILDampedCTC, self).__init__(aggr=aggr_type)
        ...
        # Durotaxis gradient amplification (Lo 2000)
        self.grad_amp_alpha = p[0, 6] if p.shape[1] > 6 else 0.0

        # CTC threshold (Wolpert 1969)
        ...
        # CIL density-dependent parameters (Mayor 2010)
        self.rho_0 = p[1, 4] if p.shape[1] > 4 and p[1, 4] != 0 else 28.0
        self.hill_n = p[1, 5] if p.shape[1] > 5 and p[1, 5] != 0 else 2.0
        ...
        print(f"initialized PDE_D_CILDampedCTC with parameters:")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_CILDampedCTC: particle_params has {n_param_rows} rows but found "
        ...
        if direction == 'interpolate':
            ...
            # Apply CIL density-dependent modulation (Mayor 2010)
            ...
            # First: compute local density for CIL
            self._compute_local_density(x, edge_index)
            # Then: compute pp forces
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            # CTC threshold coupling (Wolpert 1969)
            ...
            # NOTE: CIL modulation (Hill function) is applied post-aggregation in forward()
            return velocity_raw
