...
class PDE_D_Chiral(pyg.nn.MessagePassing):
    """
    Chiral diffusiophoresis particle model.

    Adds a gradient-perpendicular drift component to standard diffusiophoresis,
    causing particles to spiral around concentration features rather than
    moving directly up/down gradients.

    Literature:
    - Lowen, H. (2016) Eur Phys J Special Topics 225:2319-2331
      "Chirality in microswimmer motion"
    - Friedrich, B. M. & Julicher, F. (2007) PNAS 104:13256
      "Chemotaxis of sperm cells"

    Physics:
    v_total = v_parallel + chirality * rotate_90(v_parallel)
    where rotate_90_CCW(vx, vy) = (-vy, vx) in 2D.
    chirality > 0: CCW spiral; chirality < 0: CW spiral; chirality = 0: straight.
    ...
    PARAMS_DOC = {
        "model_name": "Chiral",
        "literature": "Lowen (2016) Eur Phys J Special Topics 225:2319-2331",
        "description": "Chiral diffusiophoresis with gradient-perpendicular drift",
        "equations": {
            "field_to_particle": "v = (M1*grad_C1 + M2*grad_C2) + chirality * rotate_90(M1*grad_C1 + M2*grad_C2)",
        ...
        "params_mesh": [
            ...
            {
                "row": 1, "description": "C2 field parameters + chirality control",
                "slots": [
                    ...
                    {"index": 3, "name": "unused", "description": "Unused (pad)", "typical_range": [0.0, 0.0]},
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_Chiral, self).__init__(aggr=aggr_type)
        ...
        print(f"initialized PDE_D_Chiral with parameters:")
        ...
        print(f"  chirality={ch_val:.3f} ({'CCW' if ch_val > 0 else 'CW' if ch_val < 0 else 'off'}, Lowen 2016)")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_Chiral: particle_params has {n_param_rows} rows but found "
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            # Chiral drift: rotate velocity 90 degrees and add perpendicular component
            if self.chirality != 0:
