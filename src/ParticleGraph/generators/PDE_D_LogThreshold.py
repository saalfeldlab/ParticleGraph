...
class PDE_D_LogThreshold(pyg.nn.MessagePassing):
    """
    Hybrid particle model combining logarithmic gradient sensing with
    concentration-threshold bistable coupling.

    This model combines two mechanisms:
    1. LogSensing (Kalinin 2009): Weber-Fechner law for chemotactic response
       v = M * ∇C / (C + C0)  (logarithmic gradient sensing)
    ...
    The combination enables self-limiting aggregation (from log-sensing) with
    convergent stability (from threshold coupling). Unlike DurotaxisThreshold
    which amplifies gradients, this model attenuates them at high concentrations
    while maintaining the bistable attractor at the C1=T isoline.
    ...
    PARAMS_DOC = {
        "model_name": "LogThreshold",
        "literature": "Kalinin et al. (2009) Biophys J 96:2439; Wolpert (1969) J Theor Biol 25:1",
        "description": "Logarithmic gradient sensing + bistable concentration-threshold coupling",
        "equations": {
            "field_to_particle": "v = M * ∇C/(C+C0) * (-tanh(3*(C1-T)/A)); T = ctc_threshold * A",
        ...
        "params_mesh": [
            {
                ...
                "slots": [
                    ...
                    {"index": 6, "name": "C0", "description": "Log-sensing regularization (crossover concentration)", "typical_range": [0.1, 2.0]},
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_LogThreshold, self).__init__(aggr=aggr_type)
        ...
        # Stored in params_mesh[0][6] (same slot as grad_amp_alpha in DurotaxisThreshold)
        self.C0 = p[0, 6] if p.shape[1] > 6 else 0.5
        # Ensure C0 is positive and reasonable
        if hasattr(self.C0, 'item'):
            c0_val = self.C0.item()
        else:
            c0_val = self.C0
        if c0_val <= 0:
            self.C0 = torch.tensor(0.5, device=p.device) if hasattr(p, 'device') else 0.5
        ...
        print(f"initialized PDE_D_LogThreshold with parameters:")
        print(f"  mobility: M1={self.M1.item()}, M2={self.M2.item()}")
        c0_val = self.C0.item() if hasattr(self.C0, 'item') else self.C0
        print(f"  log-sensing C0={c0_val:.3f} (crossover concentration, Kalinin 2009)")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_LogThreshold: particle_params has {n_param_rows} rows but found "
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            # Log-sensing gradient estimation: ∇C / (C + C0)
            ...
            velocity_raw = (M1 * grad_C1_log + M2 * grad_C2_log) * dir_norm
