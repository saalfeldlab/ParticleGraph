...
def taichi_MPM_debug():
    ...
    def substep_debug():
        ...
        for p in x:
            ...
            C_norm = (
                C[p][0, 0] ** 2 + C[p][0, 1] ** 2 + C[p][1, 0] ** 2 + C[p][1, 1] ** 2
            ) ** 0.5
        ...
        print(
            f"stress - Max: {max_stress_norm}, Mean: {mean_stress_norm}, Std: {std_stress_norm}"
        )
        ...
        for i, j in grid_m:
            ...
            if grid_momentum_norm > 0:
                min_grid_momentum_norm = ti.min(
                    min_grid_momentum_norm, grid_momentum_norm
                )
                max_grid_momentum_norm = ti.max(
                    max_grid_momentum_norm, grid_momentum_norm
                )
        ...
        mean_grid_momentum_norm = (
            total_grid_momentum_norm / count_grid_momentum_norm
            if count_grid_momentum_norm > 0
            else 0.0
        )
        ...
        for i, j in grid_m:
            ...
            grid_momentum_norm = ti.sqrt(grid_v[i, j][0] ** 2 + grid_v[i, j][1] ** 2)
            if grid_momentum_norm > 0:
                sum_grid_momentum_norm_sq_diff += (
                    grid_momentum_norm - mean_grid_momentum_norm
                ) ** 2

        std_grid_m = (
            ti.sqrt(sum_grid_m_sq_diff / count_grid_m) if count_grid_m > 0 else 0.0
        )
        std_grid_momentum_norm = (
            ti.sqrt(sum_grid_momentum_norm_sq_diff / count_grid_momentum_norm)
            if count_grid_momentum_norm > 0
            else 0.0
        )
        ...
        print(
            f"grid_m - Min: {min_grid_m}, Max: {max_grid_m}, Mean: {mean_grid_m}, Std: {std_grid_m}"
        )
        print(
        ...
        mean_grid_v_norm = (
            total_grid_v_norm / count_grid_v_norm if count_grid_v_norm > 0 else 0.0
        )
        ...
        std_grid_v_norm = (
            ti.sqrt(sum_grid_v_norm_sq_diff / count_grid_v_norm)
            if count_grid_v_norm > 0
            else 0.0
        )
