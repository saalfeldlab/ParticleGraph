...
class PDE_D_HollingThreshold(pyg.nn.MessagePassing):
    """
    Particle model combining Holling Type II/III functional response for
    particle-to-field coupling with durotaxis gradient amplification and
    concentration-threshold bistable coupling.

    Key innovation: Non-linear particle-to-field (pf) coupling replaces the
    standard linear consumption/production with a saturating functional response.
    At low field concentrations, consumption is weak; at high concentrations,
    it saturates. This creates a qualitatively different feedback between
    particles and fields compared to linear coupling, potentially altering
    the Turing-CTC convergence equilibrium.

    Holling Type II: consumption = c_max * C1 / (K_h + C1)
    Holling Type III: consumption = c_max * C1^n / (K_h^n + C1^n)

    Combined with:
    1. Durotaxis (Lo et al. 2000): Gradient-amplified mobility
    2. ThresholdCoupling (Wolpert 1969): Bistable CTC convergence

    Literature:
    - Holling, C.S. (1959) Canadian Entomologist 91(5):293-320
      "Components of Predation as Revealed by Small-Mammal Predation"
    - Lo, C.M. et al. (2000) Biophys J 79:144-152
    - Wolpert, L. (1969) J Theor Biol 25:1-47
    """

    PARAMS_DOC = {
        "model_name": "HollingThreshold",
        "literature": "Holling (1959) Can Entomol 91:293; Lo (2000) Biophys J 79:144; Wolpert (1969) J Theor Biol 25:1",
        "description": "Holling Type II/III functional response for pf coupling + durotaxis + CTC",
        "equations": {
            "field_to_particle": "v = M * (1+alpha*clamp(|gradC1|,max=1)) * (-tanh(3*(C1-T)/A)) * (grad_C1+grad_C2) * dir",
            "particle_to_field_typeII": "dC1 = -c_max * C1/(K_h + C1) * w(r), dC2 = production * w(r)",
            "particle_to_field_typeIII": "dC1 = -c_max * C1^n/(K_h^n + C1^n) * w(r), dC2 = production * w(r)",
        ...
        "params_mesh": [
            ...
            {
                "row": 1, "description": "C2 field parameters + Holling parameters",
                "slots": [
                    ...
                    {"index": 2, "name": "K_h", "description": "Holling half-saturation constant (consumption = c_max*C1^n/(K_h^n+C1^n))", "typical_range": [0.5, 10.0]},
                    {"index": 3, "name": "holling_n", "description": "Holling exponent: n=1 for Type II, n>=2 for Type III", "typical_range": [1.0, 3.0]},
                    {"index": 4, "name": "unused_4", "description": "Reserved", "typical_range": [0, 0]},
                    {"index": 5, "name": "unused_5", "description": "Reserved", "typical_range": [0, 0]},
                    {"index": 6, "name": "unused_6", "description": "Reserved", "typical_range": [0, 0]},
                    {"index": 7, "name": "unused_7", "description": "Reserved", "typical_range": [0, 0]}
                ]
            },
            {
                ...
                "slots": [
                    ...
                    {"index": 1, "name": "consumption_max", "description": "Max consumption rate (c_max in Holling)", "typical_range": [50, 300]},
        ...
        "particle_params": {
            ...
            "slots": [
                ...
                {"index": 2, "name": "consumption_max", "description": "Per-type max consumption rate"},
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_HollingThreshold, self).__init__(aggr=aggr_type)
        ...
        # Holling parameters
        self.K_h = p[1, 2] if p.shape[1] > 2 else 2.0
        self.holling_n = p[1, 3] if p.shape[1] > 3 else 1.0

        print(f"initialized PDE_D_HollingThreshold with parameters:")
        ...
        K_h_val = self.K_h.item() if hasattr(self.K_h, 'item') else self.K_h
        n_val = self.holling_n.item() if hasattr(self.holling_n, 'item') else self.holling_n
        c_max_val = self.consumption_max.item() if hasattr(self.consumption_max, 'item') else self.consumption_max
        if n_val >= 2.0:
            print(f"  Holling Type III: c_max={c_max_val:.1f}, K_h={K_h_val:.2f}, n={n_val:.1f} (sigmoidal)")
        else:
            print(f"  Holling Type II: c_max={c_max_val:.1f}, K_h={K_h_val:.2f} (saturating, Holling 1959)")
        ...
        print(f"  particle->field: c_max={c_max_val:.1f}, production={self.production_rate.item()}, influence_radius={self.influence_radius.item():.3f}")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_HollingThreshold: particle_params has {n_param_rows} rows but found "
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            # Holling Type II/III functional response for consumption
            # Instead of linear dC1 = -consumption * w(r),
            # use dC1 = -c_max * C1^n / (K_h^n + C1^n) * w(r)
            weights = torch.exp(-dist**2 / (2 * (self.influence_radius/3)**2))

            if parameters_i is not None:
                consumption_max = parameters_i[:, 2]
            ...
            C1_safe = torch.clamp(C1_local, min=1e-6)
            ...
            if n > 1.5:
                ...
                C1_n = C1_safe ** n
                K_h_n = K_h ** n
                holling_factor = C1_n / (K_h_n + C1_n)
            else:
                # Type II: saturating response
                holling_factor = C1_safe / (K_h + C1_safe)

            # Apply Holling response to consumption only
            # Production remains linear (inhibitor production doesn't saturate)
            field_updates = torch.zeros((pos_i.size(0), 2), device=pos_i.device)
            field_updates[:, 0] = -consumption_max * holling_factor * weights
