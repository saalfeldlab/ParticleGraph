...
class PDE_D_DurotaxisAdhesionThreshold(pyg.nn.MessagePassing):
    """
    Triple-hybrid particle model combining durotaxis gradient amplification,
    Steinberg differential adhesion, and concentration-threshold bistable coupling.
    ...
    3. CrossTypeAdhesion (Steinberg 1963): Same-type particles attract normally;
       cross-type particles experience inverted forces, driving cell sorting.

    The triple combination provides:
    - Gradient amplification accelerates convergence (Durotaxis)
    - Threshold coupling provides bistable attractor at C1=T (CTC)
    - Differential adhesion drives type-specific spatial sorting (Steinberg)
    ...
    PARAMS_DOC = {
        "model_name": "DurotaxisAdhesionThreshold",
        "literature": "Lo (2000) Biophys J 79:144; Wolpert (1969) J Theor Biol 25:1; Steinberg (1963) Science 141:401",
        "description": "Gradient-amplified mobility + bistable CTC + differential adhesion sorting",
        "equations": {
            ...
            "particle_to_particle": "f = AR_force * modifier; modifier=1 (same-type), -(1+factor) (cross-type) [Steinberg]"
        },
        "params_mesh": [
            {
            ...
            },
            {
                ...
                "slots": [
                    ...
                    {"index": 5, "name": "cross_type_factor", "description": "Controls adhesion inversion AND per-type CTC threshold spread", "typical_range": [0.0, 0.5]}
                ]
            }
        ],
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_DurotaxisAdhesionThreshold, self).__init__(aggr=aggr_type)
        ...
        # Cross-type factor: controls adhesion inversion AND per-type CTC threshold spread
        self.cross_type_factor = p[2, 5] if p.shape[1] > 5 else 0.0

        print(f"initialized PDE_D_DurotaxisAdhesionThreshold with parameters:")
        ...
        print(f"  cross_type_factor={ctf_val:.2f} (Steinberg sorting + CTC spread)")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_DurotaxisAdhesionThreshold: particle_params has {n_param_rows} rows but found "
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            if parameters_i is not None:
                ...
                # 3. Cross-type differential adhesion (Steinberg 1963):
