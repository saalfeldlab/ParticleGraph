...
class PDE_D_AnisotropicDampedCTC(pyg.nn.MessagePassing):
    """
    CTC with ANISOTROPIC field-dependent particle-particle damping.

    Extends DampedCTC by making the pp damping direction-dependent relative
    to the local field gradient. Instead of isotropically reducing all pp forces
    near the CTC threshold, this variant decomposes pp forces into components
    parallel and perpendicular to the local C1 gradient:

    - Gradient-perpendicular (tangent to isoline): STRONGER damping
      -> allows tighter packing ALONG the threshold contour
    - Gradient-parallel (normal to isoline): WEAKER damping
      -> maintains repulsion that keeps particles AT the threshold

    The aniso_ratio parameter controls the split:
      damping_perp = pp_damping * (1 + aniso_ratio)  (clamped to [0,1])
      damping_para = pp_damping * (1 - aniso_ratio)  (clamped to [0,1])
    When aniso_ratio=0, this reduces to isotropic DampedCTC.
    ...
    3. pp: ANISOTROPIC field-damped attraction-repulsion
       f_pp_perp = f_perp * (1 - damping_perp * G(C1-T))
       f_pp_para = f_para * (1 - damping_para * G(C1-T))
       where G is Gaussian envelope around threshold T

    Literature:
    - Basan, M. et al. (2013) PNAS 110:2452-2459
      "Alignment of cellular motility forces with tissue flow as a mechanism
       for efficient wound healing" — anisotropic cell-cell interactions
    ...
    PARAMS_DOC = {
        "model_name": "AnisotropicDampedCTC",
        "literature": "Basan et al. (2013) PNAS 110:2452; Szabo et al. (2006) PRE 74:061908; Wolpert (1969) JTB 25:1; Painter & Hillen (2002) CAMQ 10:501",
        "description": "CTC + anisotropic pp damping: direction-dependent force reduction near threshold isoline",
        "equations": {
            ...
            "particle_to_particle": "f_perp = f_AR_perp * (1 - damping_perp * G(C1-T)), f_para = f_AR_para * (1 - damping_para * G(C1-T))"
        },
        "params_mesh": [
            {
            ...
            {
                "row": 1, "description": "C2 field parameters + pp damping",
                "slots": [
                    ...
                    {"index": 2, "name": "pp_damping", "description": "Base pp damping strength at CTC threshold (0=off, 0.95=strong)", "typical_range": [0.0, 0.99]},
                    {"index": 3, "name": "pp_damping_width", "description": "Width of Gaussian damping zone around T (in units of A)", "typical_range": [0.1, 1.0]},
                    {"index": 4, "name": "aniso_ratio", "description": "Anisotropy ratio: 0=isotropic (=DampedCTC), 0.5=moderate, 1.0=max anisotropy", "typical_range": [0.0, 1.0]},
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_AnisotropicDampedCTC, self).__init__(aggr=aggr_type)
        ...
        # Anisotropy ratio (Basan et al. 2013)
        # 0 = isotropic (identical to DampedCTC)
        # >0 = stronger damping perpendicular to gradient (along isoline)
        self.aniso_ratio = p[1, 4] if p.shape[1] > 4 else 0.0

        print(f"initialized PDE_D_AnisotropicDampedCTC with parameters:")
        ...
        aniso_val = self.aniso_ratio.item() if hasattr(self.aniso_ratio, 'item') else self.aniso_ratio
        print(f"  aniso_ratio={aniso_val:.3f} (Basan et al. 2013)")
        print(f"    damping_perp = {min(damp_val * (1 + aniso_val), 1.0):.3f} (along isoline)")
        print(f"    damping_para = {max(damp_val * (1 - aniso_val), 0.0):.3f} (along gradient)")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_AnisotropicDampedCTC: particle_params has {n_param_rows} rows but found "
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            # Anisotropic field-dependent pp damping (Basan et al. 2013 + Painter & Hillen 2002)
            if self.pp_damping > 0 and self.ctc_threshold > 0:
                ...
                if self.aniso_ratio > 0:
                    # Compute local C1 gradient direction from neighbor field values
                    # Use the field difference between particle i and its mesh neighbor j
                    # to estimate the gradient direction at particle i's location
                    fields_i = x_i[:, 6:7]
                    fields_j = x_j[:, 6:7]
                    dC1 = (fields_j - fields_i).squeeze(1)

                    # Gradient direction estimate: dC1 * d_pos / dist gives gradient component
                    # The local gradient direction is approximated by the weighted field difference
                    grad_dir = dC1.unsqueeze(1) * d_pos / (dist_safe.unsqueeze(1) + 1e-8)
                    grad_mag = torch.sqrt(torch.sum(grad_dir**2, dim=1, keepdim=True))
                    grad_mag_safe = torch.clamp(grad_mag, min=1e-8)
                    grad_unit = grad_dir / grad_mag_safe  # [N, 2] unit vector along gradient

                    # Decompose pp forces into gradient-parallel and gradient-perpendicular
                    # f_para = (f . grad_unit) * grad_unit
                    # f_perp = f - f_para
                    f_dot_g = torch.sum(forces * grad_unit, dim=1, keepdim=True)
                    f_para = f_dot_g * grad_unit
                    f_perp = forces - f_para

                    # Anisotropic damping factors
                    damping_perp = torch.clamp(self.pp_damping * (1.0 + self.aniso_ratio), max=1.0)
                    damping_para = torch.clamp(self.pp_damping * (1.0 - self.aniso_ratio), min=0.0)

                    # Apply direction-dependent damping
                    factor_perp = 1.0 - damping_perp * gaussian_envelope.unsqueeze(1)
                    factor_para = 1.0 - damping_para * gaussian_envelope.unsqueeze(1)

                    forces = f_para * factor_para + f_perp * factor_perp
                else:
                    # Isotropic damping (identical to DampedCTC)
