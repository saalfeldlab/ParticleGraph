...
class PDE_F(pyg.nn.MessagePassing):
    ...
    def forward(self, data, continuous_field=False, continuous_field_size=None):
        ...
        if continuous_field:
            ...
            self.density = self.propagate(edge_index=edge_index, pos=pos, d_pos=d_pos, field=field,
                                          parameters=parameters, density=torch.zeros_like(x[:, 0:1]))
            density = torch.zeros_like(x[:, 0:1])
            density[continuous_field_size[0]:] = previous_density
            self.mode = 'message_passing'
            out = self.propagate(edge_index=edge_index, pos=pos, d_pos=d_pos, field=field, parameters=parameters,
                                 density=density)
        else:
            self.mode = 'kernel'
            self.density = self.propagate(edge_index=edge_index, pos=pos, d_pos=d_pos, field=field,
    ...
    def message(self, edge_index_i, edge_index_j, pos_i, pos_j, d_pos_i, d_pos_j, field_i, field_j, parameters_i,
                density_i, density_j):
        ...
        if self.mode == 'kernel':
            ...
            pressure_force = parameters_i[:, 1:2] * torch.relu((density_i + density_j) / 2 - parameters_i[:, 2:3] / 2) * \
                             self.kernel_operators['grad_triangle'] / density_j.repeat(1, 2)
            viscosity_force = parameters_i[:, 3:4] * (d_pos_j - d_pos_i) * self.kernel_operators['Gaussian'].repeat(1,
                                                                                                                    2) / density_j.repeat(
                1, 2)
