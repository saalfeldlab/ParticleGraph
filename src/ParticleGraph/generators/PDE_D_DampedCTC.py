...
class PDE_D_DampedCTC(pyg.nn.MessagePassing):
    """
    Durotaxis + CTC with field-dependent particle-particle damping.

    Extends DurotaxisThreshold by adding a damping term to pp forces when
    particles are near the CTC threshold isoline. When C1 ~ T (the target
    concentration), pp repulsion is reduced by a Gaussian damping factor,
    allowing tighter packing at the equilibrium concentration. This targets
    the convergence bottleneck where particles oscillate around the CTC
    threshold due to residual pp repulsion.
    ...
    3. pp: Field-damped attraction-repulsion
       f_pp = f_standard * (1 - damping * exp(-(C1-T)^2 / (2*width^2)))
       When C1 ~ T (particle at target), pp forces are reduced -> faster settling
    ...
    PARAMS_DOC = {
        "model_name": "DampedCTC",
        "literature": "Lo (2000) Biophys J 79:144; Wolpert (1969) J Theor Biol 25:1; Painter & Hillen (2002) CAMQ 10:501",
        "description": "Durotaxis + CTC + field-dependent pp damping near threshold isoline",
        "equations": {
        ...
        "params_mesh": [
            ...
            },
            {
                ...
                "slots": [
                    ...
                    {"index": 2, "name": "pp_damping", "description": "pp damping strength at CTC threshold (0=off, 0.5=half, 0.9=strong)", "typical_range": [0.0, 0.9]},
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_DampedCTC, self).__init__(aggr=aggr_type)
        ...
        print(f"initialized PDE_D_DampedCTC with parameters:")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_DampedCTC: particle_params has {n_param_rows} rows but found "
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            # When C1 ~ T (CTC threshold), reduce pp forces to allow settling
