...
class PDE_D_CrossRepulsionDampedCTC(pyg.nn.MessagePassing):
    """
    Durotaxis + CTC with field-dependent pp damping AND enhanced cross-type
    particle-particle repulsion.

    Extends DampedCTC by amplifying repulsive forces between particles of
    DIFFERENT types. In 3-type systems, the neutral buffer type physically
    separates active types, enabling CTC convergence. For 2-type systems
    without a buffer, this variant creates an equivalent separation force
    by making cross-type pp interactions stronger than same-type interactions.

    The cross-type repulsion factor (cross_rep_amp) multiplies the standard
    pp force when particles i and j belong to different types:
        f_pp_cross = f_pp_standard * cross_rep_amp  (when type_i != type_j)
        f_pp_same  = f_pp_standard                  (when type_i == type_j)

    This implements differential adhesion — a well-established principle in
    developmental biology where different cell types sort based on differential
    adhesion strengths (Steinberg 1963). Here, stronger cross-type repulsion
    creates effective phase separation without requiring a third buffer type.
    ...
    3. pp: Field-damped attraction-repulsion with cross-type amplification
       f_pp = f_standard * (1 - damping * exp(-(C1-T)^2 / (2*width^2)))
       f_pp *= cross_rep_amp  if type_i != type_j

    Literature:
    - Steinberg, M. S. (1963) Science 141:401-408
      "Reconstruction of tissues by dissociated cells" (differential adhesion)
    ...
    PARAMS_DOC = {
        "model_name": "CrossRepulsionDampedCTC",
        "literature": "Steinberg (1963) Science 141:401; Wolpert (1969); Lo (2000); Painter & Hillen (2002)",
        "description": "Durotaxis + CTC + pp damping + enhanced cross-type repulsion",
        "equations": {
            "field_to_particle": "v = M*(1+alpha*|gradC1|)*(-tanh(3*(C1-T)/A))*grad*dir",
            "particle_to_field": "dC1 = -consumption * w(r), dC2 = production * w(r)",
            "particle_to_particle": "f = f_AR * (1 - damping*exp(-(C1-T)^2/(2*w^2))) * cross_factor"
        },
        "params_mesh": [
            {
            ...
            {
                "row": 2, "description": "Particle-field coupling + cross-type amplification",
                "slots": [
                    ...
                    {"index": 4, "name": "cross_rep_amp", "description": "Cross-type pp repulsion amplification (1.0=no effect, >1 stronger cross-type repulsion)", "typical_range": [1.0, 5.0]},
    ...
    def __init__(self, aggr_type='mean', p=None, particle_params=None, bc_dpos=None, dimension=2, sigma=0.005):
        super(PDE_D_CrossRepulsionDampedCTC, self).__init__(aggr=aggr_type)
        ...
        # Cross-type repulsion amplification (Steinberg 1963)
        self.cross_rep_amp = p[2, 4] if p.shape[1] > 4 else 1.0

        print(f"initialized PDE_D_CrossRepulsionDampedCTC with parameters:")
        ...
        cr_val = self.cross_rep_amp.item() if hasattr(self.cross_rep_amp, 'item') else self.cross_rep_amp
        print(f"  cross_rep_amp={cr_val:.3f} (Steinberg 1963, differential adhesion)")
    ...
    def forward(self, data, direction='fp'):
        ...
        if self.particle_params is not None:
            ...
            if max_type >= n_param_rows:
                raise ValueError(
                    f"PDE_D_CrossRepulsionDampedCTC: particle_params has {n_param_rows} rows but found "
    ...
    def message(self, edge_index_i, edge_index_j, x_i, x_j, mode=None, parameters_i=None):
        ...
        if mode == 'interpolate':
            ...
            # Cross-type repulsion amplification (Steinberg 1963)
            # Amplify pp forces between particles of DIFFERENT types
            if self.cross_rep_amp > 1.0 and parameters_i is not None and x_i.numel() > 0:
                type_i = x_i[:, 1 + 2*self.dimension].long()
                type_j = x_j[:, 1 + 2*self.dimension].long()
                cross_type_mask = (type_i != type_j).float()
                # Amplify cross-type forces, leave same-type forces unchanged
                amp_factor = 1.0 + (self.cross_rep_amp - 1.0) * cross_type_mask
                forces = forces * amp_factor.unsqueeze(1)
