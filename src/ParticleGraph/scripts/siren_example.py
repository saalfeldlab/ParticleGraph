...
if __name__ == "__main__":
    ...
    for step in trange(total_steps+1):
        ...
        if not step % steps_til_summary:
            ...
            axes[2].imshow(img_grad.norm(dim=-1).cpu().view(image_width,image_width).detach().numpy())
            axes[2].set_title('Gradient')
            axes[3].imshow(img_laplacian.cpu().view(image_width,image_width).detach().numpy())
