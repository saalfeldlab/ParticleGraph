...
def data_train_cell_activity(config, erase, best_model, device):
    ...
    lr = train_config.learning_rate_start
    lr_embedding = train_config.learning_rate_embedding_start
    learning_rate_edge_embedding = train_config.learning_rate_edge_embedding
    optimizer, n_total_params = set_trainable_parameters(model=model, lr_embedding=lr_embedding, lr=lr, learning_rate_edge_embedding=learning_rate_edge_embedding)
    logger.info(f"Total Trainable Params: {n_total_params}")
    logger.info(f'Learning rates: {lr}, {lr_embedding}')
    model.train()
    ...
    for epoch in range(start_epoch, n_epochs + 1):
        ...
        for N in trange(Niter):
            ...
            for batch in range(batch_size):
                ...
                edges = torch.tensor(edges_list[k], dtype=torch.long, device=device)
...
def data_test(config=None, config_file=None, visualize=False, style='color frame', verbose=True, best_model=20, step=15, ratio=1, run=1, test_mode='', sample_embedding=False, particle_of_interest=1, device=[]):
    ...
    if 'Gcamp' in field_type:
        data_test_activity(config, config_file, visualize, style, True, best_model, step, ratio, 0, test_mode, device)
    else:
        data_test_particles(config, config_file, visualize, style, True, best_model, step, ratio, run, test_mode, sample_embedding, particle_of_interest, device)



def data_test_particles(config=None, config_file=None, visualize=False, style='color frame', verbose=True, best_model=20, step=15, ratio=1, run=1, test_mode='', sample_embedding=False, particle_of_interest=1, device=[]):
    ...
    field_type = model_config.field_type
    has_field = (field_type != '') & ('Gcamp' not in field_type) & (has_mesh_field == False) & (has_particle_field == False)
    ...
    for it in trange(start_it,start_it+800):  # start_it + min(9600+start_it,stop_it-time_step)): #  start_it+200): # min(9600+start_it,stop_it-time_step)):
        ...
        if has_activity:
            with torch.no_grad():
                pred = model(dataset_mesh, data_id=0, training=False, phi=torch.zeros(1, device=device),
                             has_field=False)
            x[:, 6:7] += pred * hnorm * delta_t
        elif model_config.mesh_model_name == 'DiffMesh':
            with torch.no_grad():
...
def data_test_activity(config=None, config_file=None, visualize=False, style='color frame', verbose=True, best_model=20, step=15, ratio=1, run=1, test_mode='', sample_embedding=False, particle_of_interest=1, device=[]):
...
def data_test(config=None, config_file=None, visualize=False, style='color frame', verbose=True, best_model=20, step=15, ratio=1, run=1, test_mode='', sample_embedding=False, particle_of_interest=1, device=[]):
    ...
    if 'Gcamp' in field_type:
        def data_test_activity(config, config_file, visualize, style, True, best_model, step, ratio, 0, test_mode, device)
