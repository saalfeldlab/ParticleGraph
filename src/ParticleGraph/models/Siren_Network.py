...
class Siren_Network_scalar(nn.Module):
    ...
    def forward(self, latent_vector=None):
        ...
        output = (1 + torch.tanh(self.net(latent_vector))) / 2
...
if __name__ == '__main__':
    ...
    model_siren = Siren_Network_scalar(in_features=1, hidden_features=64, hidden_layers=5, out_features=1, outermost_linear=True, device=device, first_omega_0=30, hidden_omega_0=30.)
    optimizer = torch.optim.Adam(model_siren.parameters(), lr=1E-2)
    model_siren.train()
    ...
    for epoch in range(10000):
        ...
        x = model_siren(age_list[sample,None])
    ...
    pred = model_siren(age_list[:,None])
    fig = plt.figure(figsize=(8, 8))
