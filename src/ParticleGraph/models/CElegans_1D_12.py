...
class CElegans_Laplacian(nn.Module):
    ...
    def __init__(self, config=None, device=None):
        ...
        self.c_ = nn.Parameter(torch.tensor(np.ones((int(self.n_datasets), 99)), device=self.device, requires_grad=True, dtype=torch.float32))
        self.d_ = nn.Parameter(torch.tensor(np.ones((int(self.n_datasets), 99)), device=self.device, requires_grad=True, dtype=torch.float32))
    ...
    def forward(self, x, data_id, frame):
        ...
        pred = alpha * laplacian_u.squeeze() + a * u.squeeze() + b * du.squeeze() + c * u.squeeze()**2 + d * du.squeeze()**2
