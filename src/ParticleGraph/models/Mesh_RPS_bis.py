...
class Mesh_RPS_bis(pyg.nn.MessagePassing):
    ...
    def __init__(self, aggr_type=None, config=None, device=None, bc_dpos=None):
        super(Mesh_RPS_bis, self).__init__(aggr=aggr_type)
        ...
        self.lin_phi_L = MLP(input_size=self.input_size, output_size=self.output_size, nlayers=self.nlayers,
    ...
    def forward(self, data=[], data_id=[], return_all=False):
        ...
        input_phi = torch.cat((uvw, embedding), dim=-1)
        input_phi_L = torch.cat((laplacian_uvw, embedding), dim=-1)

        pred = self.lin_phi(input_phi)
        pred_L = self.lin_phi(input_phi_L)

        if return_all:
            return pred+pred_L, pred, pred_L, laplacian_uvw, uvw, embedding, input_phi, input_phi_L
        else:
            return pred+pred_L
