...
class Signal_Propagation5(pyg.nn.MessagePassing):
    ...
    def __init__(self, aggr_type=None, config=None, device=None, bc_dpos=None):
        super(Signal_Propagation5, self).__init__(aggr=aggr_type)
    ...
    def forward(self, data=[], data_id=[], return_all=False, excitation=[]):
        ...
        msg = torch.matmul(self.W * self.mask, e * self.lin_edge((u-f)/(g+1E-16)))

        pred = -u + a * self.lin_edge((u-b)/(c+1E-16)) + msg
